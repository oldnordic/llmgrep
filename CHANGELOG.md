...
### Changed
- Updated `SUPPORTED_MAGELLAN_SCHEMA_VERSION` from 14 to 16 (supports Magellan v3.3.7 with `cfg_condition` column on `cfg_blocks`).
- Updated Magellan dependency to `3.3.7`.
...
### Added
- **FTS5 OR Query Mode** — Multi-word searches now use OR semantics instead of AND:
  - `"Mutex RwLock"` matches symbols containing either word (previously required both in exact order)
  - Auto-detects `symbol_fts` table availability; falls back to `LIKE` when absent
  - Tokens are double-quoted to prevent FTS5 special-character misinterpretation
  - Verified: `"test print"` returns 744 results vs 0 with previous LIKE behavior
