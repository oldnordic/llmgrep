//! Native-V3 backend implementation.
//!
//! NativeV3Backend provides the Backend trait implementation for Native-V3 databases.
//! This backend requires the native-v3 feature flag and uses the CodeGraph API.
...
/// This backend is only available when the native-v3 feature is enabled.
///
/// Note: Native-v3 databases use sqlitegraph's V3 binary format, not standard SQLite.
...
impl NativeV3Backend {
    /// Open a Native-V3 database backend.
    ///
    /// # Arguments
    ...
    pub fn open(db_path: &Path) -> Result<Self, LlmError> {
        let graph = CodeGraph::open(db_path).map_err(|e| LlmError::DatabaseCorrupted {
            reason: format!("Failed to open Native-V3 database: {}", e),
        })?;
        Ok(Self {
